...
export function ConfigCard({
  ...
  const hasRiderInfo =
    ...
    config.riderWeight;
  const userProfileHref = usernameProfileHref(config.user?.username);
  const userHandle = usernameHandle(config.user?.username);
        ...
        {showUser && !isOwner && config.user && (
          <div className="flex items-center gap-3 px-5 pt-4 pb-2 border-b border-zinc-800/50 overflow-x-auto">
            ...
            <Link
              ...
              className={`shrink-0 transition-opacity ${userProfileHref ? "hover:opacity-80" : "pointer-events-none opacity-70"}`}
